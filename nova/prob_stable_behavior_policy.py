...
class Behavior_policy:
    ...
    def init_behavior_net(self):
        ...
        for i in range(self.n_agents):
            ...
            self.mu_head.append(nn.Linear(self.args.latent_dim, self.args.latent_dim).to(self.device))
            self.logvar_head.append(nn.Linear(self.args.latent_dim, self.args.latent_dim).to(self.device))
            ...
            self.head_parameters = list(self.mu_head[i].parameters()) + list(self.logvar_head[i].parameters())

            self.behavior_optimizer.append(
                torch.optim.Adam(self.encoder_parameters + self.decoder_parameters + self.head_parameters, 
                                                    lr=self.args.lr_behavior,
                                                    eps=self.optim_eps,
    ...
    # KL divergence loss for the latent distribution
    ...
    def latent_update(self, history, encoder_hidden, prev_latent):
        ...
        for i in range(self.n_agents):
            ...
            out, encoder_hidden_per, latent_base = self.behavior_encoder[i](history_per, encoder_hidden_per)

            mu = self.mu_head[i](latent_base)
            logvar = self.logvar_head[i](latent_base)
            logvar = torch.clamp(logvar, min=self.args.logvar_min, max=self.args.logvar_max)

            if self.args.use_mean_test:
                latent = mu
            else:
                latent = self.reparameterize(mu, logvar)
            
            new_latent.append(latent.reshape(n_thread, max_vehicle_num, -1).unsqueeze(1))
            new_mu.append(mu.reshape(n_thread, max_vehicle_num, -1).unsqueeze(1))
            new_logvar.append(logvar.reshape(n_thread, max_vehicle_num, -1).unsqueeze(1))

            encoder_hidden_per = encoder_hidden_per.reshape(num_layers, n_thread, max_vehicle_num, encoder_rnn_dim)
            encoder_hidden_new.append(encoder_hidden_per.permute((1, 0, 2, 3)).unsqueeze(2))

        new_latent = torch.cat(new_latent, dim=1)
        new_mu = torch.cat(new_mu, dim=1)
        new_logvar = torch.cat(new_logvar, dim=1)

        new_latent = new_latent.cpu().detach().numpy()
        new_mu = new_mu.cpu().detach().numpy()
        new_logvar = new_logvar.cpu().detach().numpy()
        ...
        return new_latent, encoder_hidden, new_mu, new_logvar
    ...
    def learn(self, batch, t_env):
        ...
        for i in range(self.n_agents):
            ...
            for j in range(max_episode_len - 1 - self.max_history_len):
                ...
                _, encoder_hidden, latent_base = self.behavior_encoder[i](curr_history_en, encoder_hidden)

                # Get the mean and logvar for the latent distribution
                mu = self.mu_head[i](latent_base)
                logvar = self.logvar_head[i](latent_base)
                logvar = torch.clamp(logvar, min=self.args.logvar_min, max=self.args.logvar_max)
                new_latent = self.reparameterize(mu, logvar)

                # Calculate the KL divergence loss and add it to the total KL loss
            ...
            # Average the KL loss over the episode and add it to the total loss weighted by the KL weight
            kl_total = kl_total / (max_episode_len - 1 - self.max_history_len)
            loss = behavior_error + self.behavior_variation_penalty * stability_error + self.args.kl_weight * kl_total
            ...
            train_info['behavior_kl'] += kl_total.item()
